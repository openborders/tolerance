class TakeSurveysController < ApplicationController
	...
	def update
		...
		when :summary
			...
			@correctly_guessed = UserGuessesNationality.joins('INNER JOIN user_answers_surveys').joins('INNER JOIN users').where('user_guesses_nationalities.nationality_id = users.nationality_id').count()/UserGuessesNationality.all().count().to_f
